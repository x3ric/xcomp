...
#ifndef XCOMP_VERSION
#define XCOMP_VERSION "unknown"
#endif
...
/// @brief Possible backends of xcomp.
enum backend {
  BKEND_XRENDER,
  BKEND_GLX,
  BKEND_XR_GLX_HYBRID,
...
typedef struct _options_t {
  ...
  /// Whether to respect _XCOMP_SHADOW.
  ...
  // === Calculated ===
  /// Whether xcomp needs to track focus changes.
  bool track_focus;
  /// Whether xcomp needs to track window name and class.
  bool track_wdata;
  /// Whether xcomp needs to track window leaders.
  bool track_leader;
} options_t;

#ifdef CONFIG_VSYNC_OPENGL
/// Structure containing GLX-dependent data for a xcomp session.
typedef struct {
  // === OpenGL related ===
  /// GLX context.
  GLXContext context;
...
/// Structure containing all necessary data for a xcomp session.
typedef struct _session_t {
  ...
  Window overlay;
  /// Whether the root tile is filled by xcomp.
  bool root_tile_fill;
  ...
  /// Atom of property <code>_XCOMP_SHADOW</code>.
  Atom atom_xcomp_shadow;
...
/// Structure representing a top-level window xcomp manages.
typedef struct _win {
  ...
  /// The value of _XCOMP_SHADOW attribute of the window. Below 0 for
  /// none.
  long prop_shadow;

...
find_toplevel2(session_t *ps, Window wid) {
  ...
  while (wid && wid != ps->root && !(w = find_win(ps, wid))) {
    ...
    // XQueryTree probably fails if you run xcomp when X is somehow
...
map_win(session_t *ps, Window id) {
  ...
  // Notify xcomp when the shape of a window changes
  if (ps->shape_exists) {
...
/**
 * Reread _XCOMP_SHADOW property from a window.
 *
 * The property must be set on the outermost window, usually the WM frame.
 */
static void
win_update_prop_shadow_raw(session_t *ps, win *w) {
  winprop_t prop = wid_get_prop(ps, w->id, ps->atom_xcomp_shadow, 1,
      XA_CARDINAL, 32);
...
/**
 * Reread _XCOMP_SHADOW property from a window and update related
 * things.
 */
static void
win_update_prop_shadow(session_t *ps, win *w) {
...
ev_property_notify(session_t *ps, XPropertyEvent *ev) {
  ...
  // If _XCOMP_SHADOW changes
  if (ps->o.respect_prop_shadow && ps->atom_xcomp_shadow == ev->atom) {
    win *w = find_win(ps, ev->window);
    if (w)
...
#define WARNING
    ...
    "\n"
    "--refresh-rate val\n"
    "  Specify refresh rate of the screen. If not specified or 0, xcomp\n"
...
#define WARNING
    ...
    "\n"
    "--sw-opti\n"
    "  Limit xcomp to repaint at most once every 1 / refresh_rate\n"
    ...
    "\n"
    "--respect-prop-shadow\n"
    "  Respect _XCOMP_SHADOW. This a prototype-level feature, which\n"
    ...
    "  like \'50:name *= \"Firefox\"'. xcomp-trans is recommended over\n"
...
register_cm(session_t *ps) {
  ...
  // Set XCOMP_VERSION
  if (!wid_set_text_prop(ps, ps->reg_win, get_atom(ps, "XCOMP_VERSION"), XCOMP_VERSION)) {
    printf_errf("(): Failed to set XCOMP_VERSION.");
  }
...
open_config_file(char *cpath, char **ppath) {
  const static char *config_filename = "/xcomp.conf";
  const static char *config_filename_legacy = "/.xcomp.conf";
...
get_cfg(session_t *ps, int argc, char *const *argv, bool first_pass) {
  ...
  if (first_pass) {
    ...
    if (optind < argc)
      printf_errfq(1, "(): xcomp doesn't accept positional arguments.");
...
init_atoms(session_t *ps) {
  ...
  ps->atom_xcomp_shadow = get_atom(ps, "_XCOMP_SHADOW");
 ...
 *
 * This will result in xcomp resetting itself after next paint.
 */
static void
reset_enable(int __attribute__((unused)) signum) {
  session_t * const ps = ps_g;
